...
const insertIssueType = async (req,res) =>{
    ...
    try{
        await sql.connect(sqlConfig)
        const result = await sql.query(`insert into ${org}.dbo.tbl_issue_master (issue_id  ,issue_type ,issue_description  ,Status,add_user_name,add_system_name,add_ip_address,add_date_time)
...
const getIssueType = async (req,res) =>{
    ...
    try{
        await sql.connect(sqlConfig)
        const result = await sql.query(`select * from ${org}.dbo.tbl_issue_master  where sno='${sno}'`)
        res.status(200).send(result.recordset)
    }
    catch(err){
        console.log(err)
...
const deleteIssueType = async (req,res) =>{
    ...
    try{
        await sql.connect(sqlConfig)
        const result = await sql.query(`update ${org}.dbo.tbl_issue_master set status='${status}' where sno =${sno}`)
