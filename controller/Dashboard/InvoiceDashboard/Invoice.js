...
const Invoice_Outstanding = async (req, res) => {
    ...
    try {
        ...
        console.log(Vendor.recordset[0])
...
const FilterInvoice = async (req, res) => {
    ...
    try {
        await sql.connect(sqlConfig)
        const Outstanding = await sql.query(`select *,convert(varchar(15),invoice_date,105) as date from IPERISCOPE.dbo.tbl_vendor_invoice with (nolock) where ( vendor LIKE '%${value}%' or invoice_no LIKE '%${value}%' or reference_no LIKE '%${value}%' or invoice_amt LIKE '%${value}%')  order by sno ASC OFFSET (${pageno}-1)*${rowsperpage} rows FETCH next ${rowsperpage} rows only`)
